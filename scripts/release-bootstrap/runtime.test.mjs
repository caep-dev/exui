...
function fixture(t) {
  ...
  write("packages/tokens/package.json", { name: "@exre/exui-tokens", version: "0.1.0" })
...
function release(f, extra = false) {
  f.write("packages/tokens/package.json", { name: "@exre/exui-tokens", version: "0.2.0" })
...
test("planning maps a scoped package to its package-version tag", (t) => {
  const f = fixture(t)
  f.write(".changeset/feature.md", '---\n"@exre/exui-tokens": minor\n---\nFeature\n')
  const status = join(f.base, "status.json")
  writeFileSync(status, JSON.stringify({ releases: [{ name: "@exre/exui-tokens", oldVersion: "0.1.0", newVersion: "0.2.0", type: "minor", changesets: ["feature"] }] }))
  const plan = ok(f.cli("plan.mjs", "--status-file", status))
  assert.equal(plan.releases[0].tag, "@exre/exui-tokens@0.2.0")
})
test("private Changesets dependency entries with type none do not become releases", (t) => {
  ...
  writeFileSync(status, JSON.stringify({ releases: [
    { name: "@exre/exui-tokens", oldVersion: "0.1.0", newVersion: "0.2.0", type: "minor", changesets: ["feature"] },
...
test("a release commit recovers annotated tags idempotently at the same SHA", (t) => {
  ...
  assert.equal(f.git("cat-file", "-t", "refs/tags/@exre/exui-tokens@0.2.0"), "tag")
  assert.equal(ok(f.cli("tags.mjs", "--plan", path, "--commit", sha)).tags[0].state, "existing")
  const context = ok(f.cli("hook-context.mjs", "--tag", "@exre/exui-tokens@0.2.0", "--commit", sha, "--provider", "github", "--repository", "caep-dev/exui", "--hook-id", "npm", "--main-ref", "origin/main"))
  assert.equal(context.RELEASE_PACKAGE, "@exre/exui-tokens")
...
test("release subject alone cannot authorize unrelated file changes", (t) => {
  ...
  f.git("tag", "-a", "@exre/exui-tokens@0.2.0", "-m", "release")
...
test("lightweight and mismatched tags never authorize publication", (t) => {
  const f = fixture(t), sha = release(f)
  f.git("tag", "@exre/exui-tokens@0.2.0")
  const result = f.cli("hook-context.mjs", "--tag", "@exre/exui-tokens@0.2.0", "--commit", sha, "--provider", "github", "--repository", "caep-dev/exui", "--hook-id", "npm", "--main-ref", "origin/main")
  assert.notEqual(result.status, 0)
...
test("a conflicting remote tag is not moved", (t) => {
  ...
  f.git("tag", "-a", "@exre/exui-tokens@0.2.0", "-m", "wrong", old)
  f.git("push", "origin", "refs/tags/@exre/exui-tokens@0.2.0")
  ...
  assert.equal(f.git("rev-parse", "@exre/exui-tokens@0.2.0^{}"), old)
})
test("a manifest script change disguised as a version commit is rejected", (t) => {
  const f = fixture(t)
  const manifest = JSON.parse(readFileSync(join(f.root, "packages/tokens/package.json")))
  f.write("packages/tokens/package.json", { ...manifest, version: "0.2.0", scripts: { prepublishOnly: "unexpected" } })
...
test("real Changesets versioning returns JSON and limits the staged paths", (t) => {
  ...
  const path = join(f.base, "plan.json")
  writeFileSync(path, JSON.stringify({ mode: "version", releases: [{ name: "@exre/exui-tokens", oldVersion: "0.1.0", newVersion: "0.2.0" }] }))
  const result = ok(f.cli("version.mjs", "--plan", path))
  assert.deepEqual(result.changed, [".changeset/feature.md", "packages/tokens/CHANGELOG.md", "packages/tokens/package.json"])
  assert.equal(JSON.parse(readFileSync(join(f.root, "packages/tokens/package.json"))).version, "0.2.0")
