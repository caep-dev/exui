...
- For CSS Tokens, JavaScript Tokens, or component recipes, read [Token usage](references/token-usage.md). Read the [generated Token path inventory](references/generated/token-paths.md) only when exact paths or CSS custom-property names are needed.
...
- Import only from package roots or declared CSS subpaths. Never use package `src/`, `dist/`, or `types/` paths as consumer APIs.
